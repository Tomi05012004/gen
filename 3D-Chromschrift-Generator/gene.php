...
if($_SERVER["REQUEST_METHOD"] == "POST" && !empty($_POST["acti"]))
{
	...
	if($_POST["acti"] == "add")
	{
		...
		{
			conf :: addItem(sanitize_input($_POST["font"]), sanitize_input($_POST["text"]), sanitize_input($_POST["char"]), sanitize_boolean($_POST["foil"]), sanitize_boolean($_POST["true"]), sanitize_numeric($_POST["leng"]));
		}
...
echo <<<EOT
		<div id = 'foot'>
			<div style="color: grey; padding-top: 20px;" class = 'headTitl'>3D Autoaufkleber, Chrombuchstaben &  Firmenlogo-Herstellung individuell</div>
			<p>
EOT;
if($mobi)
{
echo <<<EOT
	...
	<div>
		...
		<p style = "padding: 0 0 8px 0;/* text-align: justify; padding-left: 20px; */">Alle anderen Größen und Farben bekommen Sie in unserem <a style=" color: grey;" href = 'http://www.chrombeschriftung.de' target = '_self'>MBD-ChromShop</a> unter "Einzelbuchstaben kaufen".</p>
...
else
{
	echo <<<EOT
				...
				<tr>
					...
					<td style = "vertical-align: top width: 318px;">
						<p style="text-align: justify; padding-left: 20px;">Alle anderen Größen und Farben bekommen Sie in unserem <a style=" color: grey;" href = 'http://www.chrombeschriftung.de' target = '_self'>MBD-ChromShop</a> unter "Einzelbuchstaben kaufen".</p>
					</td>
